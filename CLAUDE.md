...
### Two main app areas, one codebase

The App Router is split into two independently-laid-out sections, each with its own `layout.jsx` and nav/sidebar chrome:
...
- `src/app/admin/` — admin dashboard (products, orders, coupons management). Layout uses `src/features/admin/AdminLayout.jsx` (+ `AdminNavbar`/`AdminSidebar`), and client-side gates on `user.role === 'ADMIN'`.
- `src/app/login/`, `src/app/signup/`, `src/app/profile/` — real, working auth pages (see "Auth" below), added after the original GoCart layout was set up; they sit outside the two chrome trees above and don't use `Banner`/`Navbar`/`Footer`.

When adding a page, match the persona it belongs to (customer/admin) and place it under the corresponding tree using the existing layout/component pair — don't build new chrome from scratch.
...
- `src/app/admin/add-product/page.jsx` → `onSubmitHandler` — calls `productApi.createProduct()` → `POST /api/products` with `FormData`, uploads images
- `src/app/admin/manage-product/page.jsx` → `toggleStock` and other handlers — calls `productApi.updateProduct()` → `PUT`/`DELETE` on `/api/products/[id]`
- `src/app/admin/orders/page.jsx` → `updateOrderStatus` — calls `orderApi.updateOrder()` → `PUT /api/orders/[id]`
...
`User.role` (`UserRole`: `CUSTOMER` / `ADMIN` / `SELLER`) exists on the schema and is checked client-side in `src/features/admin/AdminLayout.jsx` (currently requires `role === 'ADMIN'`). **There is no `middleware.js` and no server-side session/route protection** — the Redux `user.current` state is just client-side UI state populated by the login flow, not a verified session, so don't assume `src/app/admin` is actually access-controlled server-side.
...
- Category lists are duplicated rather than shared: `src/assets/assets.js` exports a short `categories` list for the storefront, while `src/app/admin/add-product/page.jsx` hardcodes its own longer, different category array inline. Keep this in mind if a category list needs to change — it's not a single source of truth.
